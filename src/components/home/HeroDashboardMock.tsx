...
const SIGNALS = [
  { pair: "XAUUSD", side: "Long", state: "Target hit", tone: "bullish", when: "London" },
  { pair: "NAS100", side: "Short", state: "Invalidated", tone: "bearish", when: "New York" },
  { pair: "BTCUSD", side: "Long", state: "Running", tone: "neutral", when: "Asia" },
  { pair: "EURUSD", side: "Short", state: "Reviewed", tone: "neutral", when: "London" },
];
    ...
    <div className="product-frame mx-auto max-w-6xl">
      <div className="product-screen grid md:grid-cols-[13.5rem_1fr]">
        ...
        <aside className="hidden md:flex flex-col border-r border-white/8 p-4 gap-6">
          ...
          <div>
            <p className="px-3 mb-2 text-[0.5625rem] font-semibold tracking-[0.16em] uppercase text-muted-foreground">
              Trading
            </p>
            <MockNav items={NAV_TRADING} />
          </div>
          <div className="mt-auto mock-card p-3">
            <p className="text-[0.625rem] uppercase tracking-[0.12em] text-muted-foreground">Account</p>
            ...
            <div className="hidden sm:flex items-center gap-2">
              ...
              <span className="mock-pill !pl-1">
                ...
                <span className="hidden lg:flex flex-col leading-none">
                  ...
                  <span className="text-[0.5625rem] text-muted-foreground mt-0.5">Pro plan</span>
                </span>
              </span>
            </div>
          </div>
          ...
          <div className="p-4 sm:p-5 grid lg:grid-cols-[1.6fr_1fr] gap-4">
            <div className="mock-card p-4 sm:p-5">
              ...
              <div className="relative mt-4 pr-8">
                ...
                <div className="absolute right-0 top-2 flex flex-col gap-6 text-[0.625rem] text-muted-foreground tabular-nums">
                  <span>Hi</span>
                  <span>Mid</span>
                  <span>Lo</span>
                </div>
            ...
            <div className="mock-card p-4 sm:p-5 flex flex-col">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm font-semibold text-foreground">Recent ideas</p>
                </div>
                <span className="mock-pill !px-2" aria-hidden>
              ...
              <ul className="mt-4 flex flex-col divide-y divide-white/6">
                {SIGNALS.map(({ pair, side, state, tone, when }) => (
                  <li key={pair} className="flex items-center gap-3 py-2.5">
                    <span className="h-8 w-8 rounded-full bg-white/6 border border-white/8 flex items-center justify-center text-[0.5625rem] font-bold text-foreground">
                      {pair.slice(0, 3)}
                    </span>
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[0.625rem] text-muted-foreground">{when} session</p>
                    </div>
              ...
              <div className="mt-auto pt-3 grid grid-cols-4 gap-1.5">
                {BIAS.map(({ m, b }) => (
                  <div key={m} className="rounded-lg bg-white/4 border border-white/6 px-1.5 py-1.5 text-center">
                    <p className="text-[0.5625rem] font-bold tracking-[0.08em] text-foreground">{m}</p>
                    <p
                      className="text-[0.5625rem] capitalize"
